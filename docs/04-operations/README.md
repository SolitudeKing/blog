...
- [Runbook · 端到端部署指南](./runbook/00-deploy-from-scratch.md)：从零到对外可访问的部署 playbook（`git clone` → 配 `deploy/.env.production` → 启动 → 验收）。**生产上传走 S3**（见 `01` §10），`api-storage` 命名卷仅作开发 / 紧急回退。
- [Runbook · 部署与备份](./runbook/01-deployment-and-backup.md)：Compose、Nginx、健康检查、备份恢复、TLS、回滚原则、S3 运维（参考手册）。
- [Runbook · 制品挂载部署](./runbook/03-artifact-based-deploy.md)：默认的日常升级路径。运行时镜像仅在 alpine / nginx 升级时重建，应用制品在本机构建后 scp 到服务器，容器 bind mount 启动。
