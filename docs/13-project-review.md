...
- Compose 不再启动空异步进程；MySQL、Redis、API 调试端口仅绑定 `127.0.0.1`，敏感口令不再提供可直接运行的弱默认回退，并为常驻服务增加重启策略。
...
- MySQL 备份使用最小文件权限、临时文件、失败即停、`gzip -t` 完整性检查和同目录原子落盘；恢复要求显式确认，并在导入压缩包前检查 gzip 完整性。
...
- MySQL 与 Redis 连接配置已收敛为原子环境变量，由 Go 配置层组合 DSN 和地址；Compose 为 API 覆盖容器网络地址，环境模板不再保留无人消费的 Celery URL。
...
- 当前脚本只备份 MySQL；必须增加上传文件备份，形成数据库与媒体可对应的恢复点。
- 为备份增加独立 checksum、保留周期、异地副本、结果监控和恢复演练记录。
- 恢复前仍需自动核对 API 已停写、创建数据库与上传卷快照，并补充失败回滚流程和 SQL 语义验收。
