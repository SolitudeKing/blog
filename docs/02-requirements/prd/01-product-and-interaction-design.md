...
本文描述目标产品体验，不等同于当前功能清单。页面是否已实现、是否通过验收以 [设计进度追踪](../../03-knowledge/wiki/01-design-progress.md) 为准；精确主题色值见 [雾境主题色调系统](../../01-decisions/lld/05-theme-color-system.md)，UI 与布局的已知源码偏差分别记录在 [UI 设计系统](../../01-decisions/lld/03-ui-design-system.md#实现偏差清单) 和 [布局模式](../../01-decisions/lld/04-layout-patterns.md#16-当前实现偏差清单)。
...
- [UI 设计系统](../../01-decisions/lld/03-ui-design-system.md)：语义 token 契约、字体、间距、圆角、阴影、动效、组件 API 与状态。
- [雾境主题色调系统](../../01-decisions/lld/05-theme-color-system.md)：雾境海盐、雾境青森的 light/dark 精确映射、氛围规则与对比度基线。
- [布局模式](../../01-decisions/lld/04-layout-patterns.md)：公共页面、文章阅读、后台管理和移动端的结构与断点。
