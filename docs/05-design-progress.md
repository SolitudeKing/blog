...
| 编码实现骨架 | 进行中 | 90% | [../README.md](../README.md) | 已创建 `web`、`server`、`worker`、`deploy` 骨架；Go API 已接入 `.env` 加载、MySQL/GORM 自动迁移、Redis 健康检查、管理员初始化、JWT 登录鉴权、文章数据库优先 CRUD、站点配置持久化和公告管理；后台已接入文章管理、分类标签管理、文章分类标签选择、站点设置页和公告管理页；前台文章列表和详情阅读体验已完成 M2 收口；Compose 尚未实机校验 |
...
| M3 可日常管理 | 进行中 | 60% | [04-implementation-roadmap.md](./04-implementation-roadmap.md) | 分类标签 CRUD 和文章编辑器选择器作为 M3 前置工作已提前完成；站点配置与公告管理已完成数据库持久化、后台维护页和前台读取链路；后续继续补后台仪表盘和媒体资源管理 |
...
- 第一轮编码实现从可运行骨架开始，先完成 health、auth、user、setting、article 示例链路；当前 M2 已完成收口，M3 已正式推进。分类标签管理已作为 M3 前置工作提前完成，站点配置和公告管理已完成数据库持久化、后台界面和前台读取链路，下一步继续推进仪表盘和媒体资源管理能力。
