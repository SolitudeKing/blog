...
| M5 体验增强 | 进行中 | 20% | [04-implementation-roadmap.md](./04-implementation-roadmap.md) | 已完成 RSS 与 sitemap；下一步继续推进搜索增强、统计面板、版本记录和自动保存 |
...
- 第一轮编码实现从可运行骨架开始，先完成 health、auth、user、setting、article 示例链路；当前 M2、M3、M4 已完成代码收口，M5 已启动。分类标签管理已作为 M3 前置工作提前完成，站点配置、公告管理、后台仪表盘和媒体资源管理已完成数据库持久化、后台界面、摘要聚合、上传管理和前台读取链路；旧数据迁移、Redis 缓存、部署健康检查和备份恢复已完成；RSS 与 sitemap 已完成，下一步继续推进搜索增强、统计面板、版本记录和自动保存。
