...
具体颜色、颜色派生和主题阴影色以 [雾境二维主题色调系统](./05-theme-color-system.md) 为准；token 契约、圆角、阴影几何和组件状态以 [UI 设计系统](./03-ui-design-system.md) 为准；BEM 与工程组织以 [编码设计](./02-coding-design.md) 为准。本文只定义布局职责、尺寸关系、断点行为和可验证结果。
...
| 首页 | `pages/public/HomePage.vue`、`components/blog/ArticleCard.vue` | `_home.scss` | 7/5 作者舞台、一篇主故事 + 三条文章轨道、连续文章流、专题横条 | 860/640px 按内容优先级重排，首屏只保留一个 luminous 焦点 |
...
首页采用作者主导的编辑式舞台，不使用“作者 / feed / rail”三栏工作台。首屏唯一焦点是作者与站点身份；最近文章在下一节形成一篇媒体主故事与三条无卡片文章轨道，继续向下则回到安静的连续文章流。
...
≤ 860px：Hero 与最近文章区单列；三条文章轨道保持纵向列表
≤ 640px：全部单列，作者头像、标题和操作居中重排，主故事正文收紧
```
...
2. 第一篇文章使用带 260px 全幅主题插图的 featured story；第二至第四篇使用无独立表面的 story rail，仅靠留白和分隔线组织；第五篇起使用共享分隔的 stream 变体。
3. 文章、专题、标签、公告和社交链接都来自现有 API/setting；不得为保持构图伪造计数或文章。
4. loading、error、empty 和加载更多位于文章章节内部，不改变 Hero 尺寸，也不与旧结果重叠。
5. 小于 860px 把主故事与文章轨道重排为单列，轨道始终纵向排列；小于 640px 将主故事正文内边距收紧至 `--space-5`，DOM 顺序始终等于阅读顺序。
6. 专题入口由真实专题与标签补足到最多三项；没有数据时整个专题区隐藏，不显示假专题。
...
| 条目 | 日期 / 标题 / 专题三轴 | 日期 / 标题两轴，专题移至标题下方 |
...
登录是单任务页面，使用柔和氛围和玻璃表面。宽屏允许采用编辑式介绍栏 + 表单栏，窄屏必须回到单列；任何尺寸都不固定视口宽高。
...
| `640–767px` | 主轮廓仍为单列；首页主故事与纵向文章轨道保持阅读顺序；卡片网格由可用宽度自动决定列数 |
...
本文描述目标布局，不能据此把现有页面直接标记为已完成浏览器验收。当前实现已完成首页、壳层、搜索竞态和三类抽屉状态机的主要整理，但仍保留以下明确欠账。

| 优先级 | 当前实现 | 目标与验收 |
| --- | --- | --- |
| P1 | 文章上一篇/下一篇仍通过额外拉取最多 200 篇文章推导 | 详情 API 直接返回相邻文章，避免大归档下缺失相邻项与额外请求 |
| P2 | Topic 已使用独立公开路由与精确专题筛选；Tag 仍进入搜索筛选结果 | 只有出现明确的标签叙事需求时才新增独立路由；在此之前保留搜索查询作为唯一入口 |
...
独立于布局的组件、token 和主题偏差统一记录在 [UI 设计系统](./03-ui-design-system.md#实现偏差清单)，两份清单不得重复维护同一事实。

## 17. 本轮实施状态

1. 已完成基础布局 token、现有编辑式语义别名和海盐/青森 × light/dark 四组合映射。
...
- [ ] 首页在宽屏呈现 7/5 作者舞台与“一篇主故事 + 三条文章轨道”，860/640px 后按内容优先级单列重排。
- [ ] 文章正文行宽稳定，代码块/表格局部滚动，TOC 在 960px 前后正确切换。
- [ ] 归档年份锚点不被顶栏遮挡，320px 下日期、标题、专题不重叠。
